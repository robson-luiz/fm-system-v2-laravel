            ...
            <div class="flex items-center space-x-4">
                ...
                @can('edit-income')
                    <form method="POST" action="{{ route('incomes.toggle-status', $income) }}" class="inline-block">
                        @csrf
                        <button type="submit" class="btn-{{ $income->status === 'recebida' ? 'warning' : 'success' }}-md flex items-center space-x-1">
                            @if($income->status === 'recebida')
                ...
                @can('destroy-income')
                    <form id="delete-form-{{ $income->id }}" method="POST" action="{{ route('incomes.destroy', $income) }}">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDelete({{ $income->id }})" class="btn-danger-md flex items-center space-x-1">
