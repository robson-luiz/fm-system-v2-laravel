    ...
    <div class="content-box">
        <div class="content-box-header">
            ...
            <div class="content-box-btn">
                <a href="{{ route('incomes.show', $income) }}" class="btn-secondary-md align-icon-btn">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="received_date" id="received_date" class="form-input" 
                               value="{{ old('received_date', $income->received_date->format('Y-m-d')) }}" required>
                        @error('received_date')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="form-input" required>
                            ...
                            <option value="fixa" {{ old('type', $income->type) == 'fixa' ? 'selected' : '' }}>Receita Fixa</option>
                            <option value="variavel" {{ old('type', $income->type) == 'variavel' ? 'selected' : '' }}>Receita Variável</option>
                        </select>
                        @error('type')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="status" id="status" class="form-input" required>
                            <option value="pendente" {{ old('status', $income->status) == 'pendente' ? 'selected' : '' }}>Pendente</option>
                            <option value="recebida" {{ old('status', $income->status) == 'recebida' ? 'selected' : '' }}>Recebida</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="notes" id="notes" rows="3" class="form-input" 
                                  placeholder="Informações adicionais sobre a receita...">{{ old('notes', $income->notes) }}</textarea>
                        @error('notes')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
